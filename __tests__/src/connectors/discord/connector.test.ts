import DiscordConnector from '@/src/connectors/discord/connector';
import { ConnectorError } from '@/src/connectors/IConnector';
import { sendMessage } from '@/src/connectors/discord/utils';
import { AxiosResponse } from 'axios';

// Mock jest and set the type
jest.mock('@/src/connectors/discord/utils');
...
describe('DiscordConnector', () => {
  describe('Internal logic', () => {
    test('returns the correct ID', async () => {
      ...
      expect(connector.id).toBe('DiscordConnector');
    });
    ...
    test('throw expected error', async () => {
      ...
      try {
        ...
        if (error instanceof ConnectorError) {
          ...
          expect(error.connectorId).toBe('DiscordConnector');
        } else {
  ...
  describe('Third party interaction', () => {
    ...
    test('All requests are rejected', async () => {
      ...
      try {
        ...
        if (error instanceof ConnectorError) {
          expect(error.name).toBe('ConnectorError');
          expect(error.message).toBe('Impossible to reach the Discord service');
          expect(error.connectorId).toBe('DiscordConnector');
        } else {
